...
export const createOrder = api({ expose: true, method: "POST", path: "/sales/orders", auth: true }, async (req) => {
    ...
    const result = await ds.transaction(async (manager) => {
        ...
        const items = req.items.map((item) => manager.create(OrderItem, {
...
export const listOrders = api({ expose: true, method: "GET", path: "/sales/orders", auth: true }, async () => {
    const auth = getAuthData();
    const ds = await getDataSource();
