...
plan: "00"
subsystem: api
tags: [encore, typescript, typeorm, postgresql, offline-sync]

# Dependency graph
requires:
  - phase: 01-foundation
    provides: inventory/api.ts (createLedgerEntry), auth/middleware.ts (getAuthData, requireRole)
provides:
...
affects:
  - Phase 02 (offline sync queue relies on POST /sales/orders idempotency)
  - Phase 03 (payment integration needs order creation)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - Client-generated UUID for idempotency (OFF-01)
    - Delta-based inventory deduction (OFF-04)
    - Cross-service ledger entry with client_generated_id for double idempotency

key-files:
  created:
    - sales/encore.service.ts — Service + SQLDatabase declaration
    - sales/datasource.ts — TypeORM DataSource with Order + OrderItem
    - sales/entities.ts — Order (PrimaryColumn uuid) and OrderItem entities
    - sales/migrations/1_create_sales.up.sql — Schema with indexes and constraints
    - sales/api.ts — createOrder + listOrders endpoints
...
patterns-established:
  - "Service-per-domain: sales/ follows inventory/ pattern exactly"
  - "TypeORM with synchronize:false, Encore manages migrations"
  - "getAuthData()! non-null assertion for auth:true endpoints"
