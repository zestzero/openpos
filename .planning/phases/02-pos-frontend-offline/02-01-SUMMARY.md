...
plan: "01"
subsystem: ui
tags: [vite, react, tanstack-router, tailwind, shadcn-ui, pwa, typescript]

# Dependency graph
requires:
  - phase: "01-foundation-backend-core"
    provides: "Backend services (auth, catalog, inventory) with REST APIs — frontend will call these via Encore client"
provides:
...
affects:
  - "02-pos-frontend-offline (subsequent plans in this phase)"
...
tech-stack:
  added:
    ...
    - shadcn (button, card, input, sheet, badge, tabs, dialog, sonner)
    - lucide-react ^0.500.0
    - class-variance-authority ^0.7.1
    - clsx ^2.1.0
    - tailwind-merge ^3.0.0
  patterns:
    - "File-based routing via TanStack Router with routeTree.gen.ts auto-generation"
    - "shadcn/ui copy-paste ownership model (components live in src/components/ui/)"
    - "Tailwind v4 CSS-first config via @theme directive in globals.css"
    - "PWA manifest-first offline strategy (service worker deferred to later phase)"

key-files:
  created:
    - "frontend/package.json — dependencies and scripts"
    - "frontend/vite.config.ts — Vite + TanStack Router + Tailwind v4 plugins"
    - "frontend/tsconfig.json — ES2022 target, bundler module resolution, @/* aliases"
    - "frontend/index.html — PWA manifest link, Inter + Geist fonts"
    - "frontend/public/manifest.json — PWA metadata (start_url: /pos, standalone display)"
    - "frontend/src/main.tsx — RouterProvider entry with routeTree"
    - "frontend/src/routes/__root.tsx — Root layout with Outlet"
    - "frontend/src/routes/index.tsx — Redirect / → /pos"
    - "frontend/src/routes/pos/route.tsx — POS layout (min-h-dvh bg-background)"
    - "frontend/src/routes/pos/index.tsx — POS index placeholder"
    - "frontend/src/lib/utils.ts — cn() utility (clsx + twMerge)"
    - "frontend/src/styles/globals.css — Tailwind v4 @import + @theme tokens"
    - "frontend/components.json — shadcn/ui config (New York, Zinc, lucide)"
    - "frontend/src/components/ui/* — 8 installed components"
  modified: []
...
patterns-established:
  - "Pattern: Route files in src/routes/* auto-generate route tree via TanStack Router Vite plugin"
  - "Pattern: shadcn components owned in src/components/ui/ — not node_modules"
  - "Pattern: Tailwind v4 design tokens defined in @theme block of globals.css"
